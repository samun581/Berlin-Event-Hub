...
async function register() {
  ...
  if (cleanName.length < 2 || cleanName.includes('@') || /\d/.test(cleanName) || !/^[\p{L}][\p{L}\p{M} .'-]*$/u.test(cleanName)) {
    errorMessage.value = 'Please enter a valid name using letters, spaces, apostrophes or hyphens.'
    return
  }
  ...
  try {
    const response = await fetch(apiUrl('/api/register'), {
      ...
      body: JSON.stringify({ name: cleanName, email: email.value.trim(), password: password.value }),
    })
