...
function availabilityText(event) {
  ...
  return `${event.availablePlaces} places left`
}
...
onMounted(async () => {
  try {
    ...
    upcomingEvents.value = Array.isArray(events) ? events.slice(0, 6) : []
  } catch (error) {
    console.error(error)
  } finally {
    loading.value = false
      ...
      <div class="card-grid">
        <article v-for="event in upcomingEvents" :key="event._id" class="event-card compact image-card">
          ...
          <div class="event-card-body">
            ...
            <h3>{{ event.title }}</h3>
            <p class="muted">{{ formatDate(event.date) }} · {{ event.time || 'Time not set' }} · {{ event.location }}</p>
