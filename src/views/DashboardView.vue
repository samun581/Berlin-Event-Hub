...
async function updateProfile() {
  ...
  const cleanName = profile.value.name.trim()
  if (cleanName.length < 2 || cleanName.includes('@') || /\d/.test(cleanName) || !/^[\p{L}][\p{L}\p{M} .'-]*$/u.test(cleanName)) {
    profileError.value = 'Please enter a valid name using letters, spaces, apostrophes or hyphens.'
    return
  }
  try {
    const response = await authFetch('/api/profile', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: profile.value.name.trim() }) })
    const data = await response.json()
    if (!response.ok) return (profileError.value = data.message || 'Could not update profile.')
    profileMessage.value = data.message
    profile.value.name = cleanName
...
async function changePassword() {
  ...
  if (!currentPassword.value || !newPassword.value) return (passwordError.value = 'Enter your current and new password.')
  if (!Object.values(passwordChecks.value).every(Boolean)) return (passwordError.value = 'Please meet all password requirements.')
  if (newPassword.value !== confirmNewPassword.value) return (passwordError.value = 'New passwords do not match.')
  if (currentPassword.value === newPassword.value) return (passwordError.value = 'New password must be different from your current password.')
  try {
    ...
    passwordMessage.value = `${data.message} Please log in again.`
    currentPassword.value = ''; newPassword.value = ''; confirmNewPassword.value = ''; showPasswordForm.value = false
    localStorage.removeItem('token')
    localStorage.removeItem('role')
    localStorage.removeItem('name')
    window.setTimeout(() => window.location.assign('/login?reason=password-changed'), 900)
...
async function cancelBooking(booking) {
  ...
  try {
    const response = await authFetch(`/api/bookings/${booking._id}`, { method: 'DELETE' })
    const data = await response.json()
  ...
  <main>
    ...
    <div class="dashboard-grid">
      ...
      <section>
        ...
        <p class="muted">Change your password only when you need to. If you forget it, use Forgot password? on the Login page.</p>
        <p v-if="passwordMessage" class="message success">{{ passwordMessage }}</p>
        <p v-if="passwordError" class="message error">{{ passwordError }}</p>
        <button v-if="!showPasswordForm" class="secondary" @click="showPasswordForm = true">Change Password</button>
        <div v-else>
          <div class="form-field"><label for="currentPassword">Current Password</label><input id="currentPassword" v-model="currentPassword" type="password" autocomplete="current-password" /></div>
          <div class="form-field">
            <label for="newPassword">New Password</label><input id="newPassword" v-model="newPassword" type="password" autocomplete="new-password" />
          ...
          <div class="form-field"><label for="confirmNewPassword">Confirm New Password</label><input id="confirmNewPassword" v-model="confirmNewPassword" type="password" autocomplete="new-password" /></div>
          <div class="action-row"><button @click="changePassword">Save New Password</button><button class="secondary" @click="showPasswordForm = false; currentPassword = ''; newPassword = ''; confirmNewPassword = ''; passwordError = ''">Cancel</button></div>
        </div>
      </section>
    </div>
      ...
      <div class="booking-grid">
        <article v-for="booking in bookings" :key="booking._id" class="booking-card image-card">
          ...
          <div class="event-card-body">
            ...
            <button class="danger" :disabled="booking.event.isPast" @click="cancelBooking(booking)">{{ booking.event.isPast ? 'Past Event' : 'Cancel Booking' }}</button>
          </div>
        </article>
      </div>
    </section>
