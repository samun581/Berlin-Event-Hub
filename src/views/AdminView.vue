...
async function loadStats() {
  try {
    const response = await authFetch('/api/admin/stats', { headers: adminHeaders() })
...
async function loadBookings() {
  try {
    const response = await authFetch('/api/admin/bookings', { headers: adminHeaders() })
    const data = await response.json()
...
async function loadEvents() {
  loading.value = true
  try {
    const response = await authFetch('/api/events?includePast=true')
...
async function addEvent() {
  ...
  try {
    const response = await authFetch('/api/events', { method: 'POST', headers: adminHeaders(true), body: JSON.stringify(newEvent.value) })
    const data = await response.json()
...
async function saveEdit(id) {
  message.value = ''; errorMessage.value = ''
  try {
    const response = await authFetch(`/api/events/${id}`, { method: 'PUT', headers: adminHeaders(true), body: JSON.stringify(editingEvent.value) })
    const data = await response.json()
...
async function deleteEvent(event) {
  ...
  message.value = ''; errorMessage.value = ''
  try {
    const response = await authFetch(`/api/events/${event._id}`, { method: 'DELETE', headers: adminHeaders() })
    const data = await response.json()
...
async function sendReminder(event) {
  ...
  message.value = ''; errorMessage.value = ''
  try {
    const response = await authFetch(`/api/admin/events/${event._id}/reminders`, { method: 'POST', headers: adminHeaders() })
    const data = await response.json()
    ...
    <section>
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            <tr v-for="item in stats.bookingsByEvent" :key="item.eventId"><td>{{ item.title }}</td><td>{{ item.bookings }}</td><td>{{ item.capacity == null ? 'Not set' : item.capacity }}</td><td>{{ item.capacity == null ? 'Capacity unavailable' : `${Math.max(item.capacity - item.bookings, 0)} left` }}</td></tr>
        ...
        <article v-for="event in events" :key="event._id" class="admin-event-card">
          ...
          <div v-else class="admin-event-display">
            ...
            <div class="admin-event-copy">
              <div class="card-topline"><span class="badge">{{ event.category }}</span><span :class="['availability', { sold: event.isSoldOut || event.isPast }]">{{ adminAvailability(event) }}</span></div>
              <h3>{{ event.title }}</h3>
