...
function formatTime(time) { return time || 'Time not set' }
...
function availabilityText(event) {
  ...
  if (event.capacity == null || event.availablePlaces == null) return 'Capacity unavailable'
  if (event.isSoldOut) return 'Sold out'
  return `${event.availablePlaces} of ${event.capacity} places left`
}
...
async function bookEvent(event) {
  ...
  if (!token) {
    router.push({ path: '/login', query: { reason: 'booking', redirect: `/events/${event._id}` } })
    return
  }
    ...
    <div class="event-list">
      <article v-for="event in filteredEvents" :key="event._id" class="event-card event-detail-card">
        ...
        <div class="event-card-body">
          ...
          <p class="muted">Open View Details to see the event map.</p>
        </div>
      </article>
    </div>
  </main>
