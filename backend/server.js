  ...
  try {
    ...
    if (
      ...
      return res.status(403).json({
        message: 'Your session expired after a password change. Please log in again.',
      })
    }
...
function nameValidationMessage(name) {
  ...
  if (name.includes('@') || /\d/.test(name)) {
    return 'Please enter your name, not an email address or number.'
  }
...
function eventDateTimeKey(event) {
  ...
  if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(String(event.time || ''))) return null
...
function isEventPast(event) {
  ...
  return key ? key <= berlinNowKey() : false
}
...
function validateEvent(event) {
  if (
    ...
    !event.time ||
    !event.location ||
    Number.isNaN(event.price) ||
    Number.isNaN(event.capacity)
  ) {
...
async function eventWithAvailability(db, event) {
  ...
  const availablePlaces =
    capacity === null ? null : Math.max(capacity - bookedCount, 0)

  return {
    ...event,
    description: event.description || 'Event details unavailable.',
    ...
    bookedCount,
    availablePlaces,
    isSoldOut: capacity !== null && bookedCount >= capacity,
...
async function sendAutomaticReminders(db) {
  const now = new Date()
  ...
  for (const event of upcomingEvents) {
    ...
    const hoursUntilEvent =
      (eventAt.getTime() - now.getTime()) / 3600000

    if (
      !Number.isFinite(hoursUntilEvent) ||
      hoursUntilEvent <= 0 ||
      hoursUntilEvent > 24
    ) {
      continue
    ...
    for (const booking of bookings) {
      ...
      try {
        await sendEmail({
          ...
          text: `Reminder: You booked ${event.title} on ${event.date} at ${event.time} at ${event.location}. We look forward to seeing you.`,
          html: `<h2>Event reminder</h2><p>Hello ${user.name || 'there'},</p><p>This is a reminder for your booking:</p><p><strong>${event.title}</strong><br>Date: ${event.date}<br>Time: ${event.time}<br>Location: ${event.location}</p>`,
        })
        ...
        console.error('Automatic reminder failed:', error.message)
      }
    }
  }
}
...
async function startServer() {
  try {
    ...
    app.get('/api/health', (req, res) => {
      res.json({
        message: 'Berlin Event Hub API is running',
      })
    })
    ...
    app.get('/api/events', async (req, res) => {
      try {
        ...
        for (const event of events) {
          enrichedEvents.push(
            await eventWithAvailability(db, event)
          )
        }
        ...
        res.json(
          includePast
            ? enrichedEvents
            : enrichedEvents.filter((event) => !event.isPast)
        )
      } catch (error) {
        console.error(error)
      ...
      async (req, res) => {
        try {
          ...
          if (event.capacity < bookedCount) {
            return res.status(400).json({
              message: `Capacity cannot be lower than the current ${bookedCount} booking(s).`,
            })
          }
      ...
      try {
        ...
        if (!user) {
          return res.json({
            ...
            deliveryMode: configuredDeliveryMode,
          })
        }
        ...
        let deliveryMode = configuredDeliveryMode
        ...
        try {
          const emailResult = await sendEmail({
            ...
            text:
              ...
              `If the link is not clickable, copy and paste the complete URL into the browser on the computer running Berlin Event Hub.`,
            ...
            html: `
              ...
              <p>
                If the link above is not clickable, copy and paste the complete URL below into the browser on the computer running Berlin Event Hub:
              </p>
        ...
        if (
          configuredDeliveryMode === 'preview' &&
      ...
      async (req, res) => {
        try {
          ...
          if (isEventPast(event)) {
            return res.status(400).json({
              message:
                'This event has already started and cannot be booked.',
            })
          }
          ...
          if (
            ...
            return res.status(400).json({
              message:
                'This event is missing booking information. Please contact the administrator.',
            })
          }
          ...
          if (user?.email) {
            try {
              const emailResult = await sendEmail({
                ...
                text: `Your booking for ${event.title} on ${event.date} at ${event.time || 'Time not set'} at ${event.location} is confirmed.`,
                html: `<h2>Booking confirmed</h2><p><strong>${event.title}</strong></p><p>Date: ${event.date}<br>Time: ${event.time}<br>Location: ${event.location}</p><p>We look forward to seeing you.</p>`,
              })
    ...
    app.get(
      ...
      async (req, res) => {
        try {
          ...
          for (const booking of bookings) {
            if (!ObjectId.isValid(booking.eventId)) {
              continue
            }
      ...
      async (req, res) => {
        try {
          ...
          if (isEventPast(event)) {
            return res.status(400).json({
              message:
                'Reminders cannot be sent for an event that has already started.',
            })
          }
          ...
          if (!event.time) {
            return res.status(400).json({
              message:
                'Add an event time before sending reminders.',
            })
          }
          ...
          if (bookings.length === 0) {
            return res.json({
              message:
                'There are no booked users to remind yet.',
              sent: 0,
              previewed: 0,
              failed: 0,
            })
          ...
          for (const booking of bookings) {
            if (!ObjectId.isValid(booking.userId)) {
              continue
            }
            ...
            try {
              const result = await sendEmail({
                to: user.email,
                subject: `Reminder: ${event.title}`,
                text: `Reminder: You booked ${event.title} on ${event.date} at ${event.time || 'Time not set'} at ${event.location}. We look forward to seeing you.`,
                html: `<h2>Event reminder</h2><p>Hello ${user.name || 'there'},</p><p>This is a reminder for your booking:</p><p><strong>${event.title}</strong><br>Date: ${event.date}<br>Time: ${event.time}<br>Location: ${event.location}</p>`,
              })
          ...
          const modeText =
            SENDGRID_API_KEY && FROM_EMAIL
              ? `${sent} email reminder(s) sent.`
              : `${previewed} reminder preview(s) generated in the backend terminal. Configure SendGrid to send real emails.`
          ...
          res.status(500).json({
            message:
              'Could not send event reminders.',
          })
        }
      }
    )
    ...
    sendAutomaticReminders(db).catch((error) =>
      console.error(
        'Automatic reminder check failed:',
        error.message
      )
    )
